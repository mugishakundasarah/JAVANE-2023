...
const Signup = () => {
        ...
        switch (name) {
            case "email":
              errors.email = validEmailRegex.test(value) ? '' : "Email is not valid"
                break;
            case "mobile":
              errors.mobile = value.length != 10 ? ' Phone number must be 10 characters long ' : '';
            break;
          default:
            break;
        }
    ...
    const handleSubmit = async() => {
        try {
            setLoading(true)
            let submitData = await axios.post(`${BASE_URL}/users/register/as-standard`, registrationData);
            if(submitData.status == 201){
                toast.success(submitData.data.message);
                setLoading(false)
                navigate('/')
            }else{
                toast.error(submitData.data.data.message);
                setLoading(false)
            }
        } catch (error) {
                ...
                <div className="w-[100%] mt-12">
                    ...
                    <div className="mt-6">
                        <Input 
                            ...
                            inputHandler={inputHandler}
                            required={true}
                            defaultInputValue={registrationData.mobile}
                            disabled={loading}
